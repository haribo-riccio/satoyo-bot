...
S&P 500 指数と為替レート（TTM）から円建て S&P500 投信の理論値を計算し、Discord に通知する BOT。

## 機能

- S&P 500 終値の取得（Yahoo Finance Japan）
- TTS/TTB 為替レートの取得（外為オンライン）
- 円建て理論値の前日比・年初来損益を計算
...
| データ      | ソース              | 形式                |
| ----------- | ------------------- | ------------------- |
| S&P500 終値 | Yahoo Finance Japan | HTML スクレイピング |
...
投資信託の基準価額更新ルールに基づき、日本市場と米国市場の休場状態に応じて処理を分岐:

| 本日国内市場       | 前日米国市場 | 動作                              |
| ------------------ | ------------ | --------------------------------- |
| 休場（週末・祝日） | -            | 基準価額更新なし（通知スキップ）  |
| 稼働               | 稼働         | 通常処理（前日 SP500 + 本日 TTM） |
| 稼働               | 休場         | 前々日 SP500 + 本日 TTM で計算    |

- **国内市場休場日**: 週末および日本の祝日（Holidays JP API で判定）
- **米国市場休場日**: NYSE 休場日（コード内に静的定義）
